#!/usr/bin/env node
/**
 * Seed script: populates Supabase with sample vehicles, testimonials, and clients.
 *
 * Usage:
 *   VITE_SUPABASE_URL=https://xxx.supabase.co VITE_SUPABASE_ANON_KEY=xxx node scripts/seed-sample-data.mjs
 *
 * Requires @supabase/supabase-js:  npm i @supabase/supabase-js
 */
...
// ─── Real car images from Unsplash (free license) ──────────────
const VEHICLES = [
  {
  ...
  },
  {
    ...
    media: [{ url: 'https://images.unsplash.com/photo-1606611013016-969c19ba27a5?w=800&h=600&fit=crop', is_primary: true, alt_text: '2024 Honda Civic RS Rallye Red' }],
  },
  {
...
const TESTIMONIALS = [
  { name: 'Chioma Eze', company: 'Lagos Business School', rating: 5, quote: 'Empathon Autos made my first car purchase seamless. From selection to delivery, every step was transparent and professional. I drove home in a brand new Camry within a week.', is_published: true, sort_order: 1 },
  { name: 'Ahmed Bello', company: 'Kano Chamber of Commerce', rating: 5, quote: 'We sourced our entire fleet through Empathon. Their corporate sales team understood our needs and delivered beyond expectations. Five SUVs, all delivered on time.', is_published: true, sort_order: 2 },
  { name: 'Ngozi Eze', company: null, rating: 5, quote: 'I was nervous about importing a car but they handled everything. I just showed up and drove away. The pre-order process was surprisingly smooth and transparent.', is_published: true, sort_order: 3 },
  { name: 'Tunde Adesanya', company: 'TechBridge Solutions', rating: 5, quote: 'Best car buying experience in Lagos. No haggling, no hidden fees. They gave me a fair price and the car was exactly as described. Will definitely buy from them again.', is_published: true, sort_order: 4 },
  { name: 'Funke Akindele', company: null, rating: 4, quote: 'Professional, reliable, and honest. Empathon found me the exact spec I wanted through their pre-order service. The whole process took about 6 weeks from deposit to delivery.', is_published: true, sort_order: 5 },
  { name: 'Emeka Obi', company: 'Dangote Industries', rating: 5, quote: 'As a corporate fleet manager, I need reliability and consistency. Empathon has been our go-to for 3 years now. Their after-sales support is genuinely excellent.', is_published: true, sort_order: 6 },
]
...
async function seed() {
  ...
  for (const v of VEHICLES) {
    ...
    const { data: inserted, error } = await supabase.from('vehicles').insert(vehicleData).select('id').single()
    if (error) { console.error(`  ✗ ${v.make} ${v.model}:`, error.message); continue }
    if (media?.length) {
      const mediaRows = media.map((m, i) => ({ vehicle_id: inserted.id, type: 'image', url: m.url, sort_order: i, is_primary: m.is_primary, alt_text: m.alt_text }))
      const { error: me } = await supabase.from('vehicle_media').insert(mediaRows)
  ...
  console.log('\n🏢 Seeding clients (content_blocks)...')
  const existing = await supabase.from('content_blocks').select('id').eq('page_key', 'home').eq('title', 'clients').single()
  if (existing.data) {
    const { error } = await supabase.from('content_blocks').update({ body: JSON.stringify(CLIENTS) }).eq('id', existing.data.id)
    if (error) console.error('  ✗ Update clients:', error.message); else console.log('  ✓ Updated clients block')
  } else {
    const { error } = await supabase.from('content_blocks').insert({ page_key: 'home', title: 'clients', body: JSON.stringify(CLIENTS) })
